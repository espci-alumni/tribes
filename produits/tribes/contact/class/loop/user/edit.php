...
class extends loop_sql
{
	...
	function __construct($contact_id, $form)
	{
		...
		$sql = "SELECT {$this->table}_id AS id, is_active, is_obsolete, admin_confirmed, contact_confirmed, contact_confirmed_data
				FROM contact_{$this->table}
				WHERE contact_id={$contact_id} AND is_obsolete<=0 AND contact_confirmed_data!=''
				ORDER BY sort_key";
	...
	{
		$o = (object) ((array) $o + (array) unserialize($o->contact_confirmed_data));
		...
		unset($o->contact_confirmed_data);
